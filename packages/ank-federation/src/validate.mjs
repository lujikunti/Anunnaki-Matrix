const CONTRACT_VERSION = "2026-09-25.1";
...
export class FederationValidationError extends Error {
  constructor(code, message, details=[]) {
    super(message); this.name="FederationValidationError"; this.code=code; this.details=details;
  }
}

const isObj = v => v && typeof v === "object" && !Array.isArray(v);
const req = (o,k,e) => { if(!isObj(o) || o[k]===undefined || o[k]===null || o[k]==="") e.push("missing:"+k); };

export function validateFederationEnvelope(envelope, {receivingProduct=null}={}) {
  const errors=[];
  if(!isObj(envelope)) throw new FederationValidationError("INVALID_ENVELOPE","Envelope must be an object");
  for(const k of ["schema_version","message_id","correlation_id","issued_at","issuer","audience","message_type","authority","data_classification","payload"]) req(envelope,k,errors);
  ...
  const type=envelope.message_type;
  const required={
  ...
  if(type==="TEACHING_OFFER") {
    ...
    )) errors.push("teaching:network_export_not_safe");
  }
...
export function buildEnvelope({issuer,audience,messageType,payload,authority,dataClassification="INTERNAL",messageId,correlationId,issuedAt}) {
  const env={schema_version:"1.0",message_id:messageId,correlation_id:correlationId,issued_at:issuedAt||new Date().toISOString(),issuer,audience,message_type:messageType,authority,data_classification:dataClassification,payload};
  validateFederationEnvelope(env);
  return env;
}
